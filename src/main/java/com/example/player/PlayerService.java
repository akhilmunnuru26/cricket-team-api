...
public class PlayerService implements PlayerRepository {
    ...
    public Player addPlayer (Player playerDetails){
        playerDetails.setPlayerId(uniquePlayerId);
        team.put(uniquePlayerId, playerDetails);
        uniquePlayerId+=1;
    ...
    @Override
    public Player updatePlayerInfo(int playerId, Player playerDetails){
        ...
        if(playerDetails.getJerseyNumber() !=  -1){
    ...
    @Override
    public void deletePlayerInfo(int playerId){
        Player player = team.get(playerId);
        if(player ==  null){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }else{
